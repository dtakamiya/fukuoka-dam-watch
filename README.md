...
9ダム個別カード（円形リングゲージ・貯水量・前日同時間比・安全/注意/警戒/危険の 4 段階色分け）／観測時刻と取得元の表示／
...
前日同時間比は貯水率のみ 1 系統に集約し、数値の下に「前日同時間比 ▲+0.2pt」相当を出す（▲上昇＝緑 /
▼下降＝赤 / →横ばい＝グレー）。色に依存しないよう形状アイコンと `aria-label`（例「前日同時間比 貯水率
