...
9ダム合計サマリ＋ダム別カード（貯水率で色分け）。各カードと合計サマリでは、大きな貯水率の数字の
すぐ隣に前時点比の方向グリフ（▲上昇＝緑 / ▼下降＝赤 / →横ばい＝グレー＋差分の絶対値）を大きく
表示し、増減が一目で分かるようにする。加えて各カード下部のチップ行に、1時間前の正時からの
増減インジケータ（貯水量・貯水率それぞれの符号付き差分値）を残す。
横ばい判定のしきい値は |Δ貯水率| < 0.1pt、|Δ貯水量| < 1千m³。色に依存しないよう
形状アイコンと `aria-label`（例「貯水率 上昇 0.3ポイント」）でも増減を伝える。
左端の色帯は貯水率ステータス（平常／注意／渇水）で、増減とは独立。
