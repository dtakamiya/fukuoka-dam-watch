...
| `wl-dam-04` | 現在値ビュー（9ダム個別カード＋合計サマリ・前日同時間比・色分け） | ✅ |
...
9ダム個別カード（円形リングゲージ・貯水量・前日同時間比・前月同時間比・安全/注意/警戒/危険の 4 段階色分け）／観測時刻と取得元の表示／
...
> 境界値 70 / 50 / 30 は表示用の仮値で、公的な根拠はない（下記「しきい値の根拠」参照）。

### しきい値の根拠

**結論: 70 / 50 / 30% は仮値のまま。福岡市等が公表する取水制限・渇水対策の貯水率基準としての裏取りはできなかった（2026-09 調査）。**
警戒・危険の表示は公式の発動ラインではなく、あくまで表示上の目安。

確認した公表資料（いずれも貯水率の数値基準は記載なし）:

- [福岡市 渇水対策会議を解散しました](https://www.city.fukuoka.lg.jp/mizu/somu/shisei/save-water-action_5.html) — 2026-01-28 設置・07-13 解散の事実のみ
- [福岡地区水道企業団 渇水の記録](https://www.f-suiki.or.jp/waterquality-info/kassui-record/) — 過去の渇水・送水制限の実績表
- [RKB毎日放送 2024-02 報道](https://rkb.jp/contents/202402/202402160199/) — 貯水率 32.6% 時点で制限率 10% を実施（制限率のみで発動基準の記載なし）

参考（基準ではなく実績）: 2026-01 は筑後川水系6ダムが貯水率 20.1%（1/30）まで低下し、1/29 から 30% の取水制限が実施された。
今後、公式の基準が確認できればその値に差し替える。

### 現在値ビュー

情報階層は「合計リング＋状態ラベル → 9ダム 3 列グリッド → 推移グラフ」。合計サマリはリングに加えて
内訳セグメントバー（安全／注意／警戒／危険の本数）と直近 30 日のスパークラインを持つ。
前日同時間比・前月同時間比は貯水率のみ 1 系統に集約し、数値の下に「前日同時間比 ▲+0.2pt」
「前月同時間比 ▼-1.5pt」相当を2行で出す（▲上昇＝緑 / ▼下降＝赤 / →横ばい＝グレー）。
...
- 切り出しは点数でなく **最新観測からの時刻差**（24h/7d/30d は従来どおり 24/168/720 点）。`history.json` は直近35日が毎正時、
  それ以前が日次に間引かれる（保持ポリシー参照）ため、点数ベースだと混在後に範囲がずれるので時刻ベースに変更した。
- 90日・1年はフロントで **各 JST 暦日 1 点**（12:00 に最も近い観測。最新日のみ現在値と揃えるため最新観測）に集約して描画。
  時間粒度と日次点が混在しても 1 日 1 点になる。データ生成仕様は変えていない。
- 軸ラベルは月初が「M月」、それ以外は「M/D」。月境界を優先して間引く（1年は 2 か月おき等）。ツールチップは日付を完全表記。
- 保有データが期間に満たないときは取得済みの範囲だけ描き、「データが不足しています」注記を出す。

| 1年（PC・400日分の合成データ） | 90日（幅375px） | 1年でデータ不足（幅375px・実データ25日分） |
| --- | --- | --- |
| ![1年（PC）](docs/screenshots/long-range-1y-pc.png) | ![90日（スマホ）](docs/screenshots/long-range-90d-mobile.png) | ![データ不足注記](docs/screenshots/long-range-shortage-mobile.png) |
